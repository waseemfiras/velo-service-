...
const reportsStartStr = `                    ) : (
                      <motion.div
                        key="reports_tab"`;

const newReportsStartStr = `                    ) : activeTab === 'reports' ? (
                      <motion.div
                        key="reports_tab"`;

code = code.replace(reportsStartStr, newReportsStartStr);

const reportsEndStr = `                          </div>
                        )}
                      </motion.div>
                    )}
                  </AnimatePresence>
                            ...
                            {usersList.map((user) => (
                              ...
                              >
                                ...
                                <div className="flex gap-2 w-full sm:w-auto">
                                  <button
                                    onClick={async () => {
                                      await localToggleAdmin(user.email);
                                      setUsersList(prev => prev.map(u => u.email === user.email ? { ...u, isAdmin: !u.isAdmin } : u));
                                    }}
                                  ...
                                  <button
                                    onClick={async () => {
                                      ...
                                      await localDeleteUser(user.email);
                                      setUsersList(prev => prev.filter(u => u.email !== user.email));
                                      setDeletingId(null);
                                    }}
...
code = code.replace(reportsEndStr, usersTabContent);
