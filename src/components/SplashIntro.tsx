...
export function SplashIntro({ onComplete }: SplashIntroProps) {
  useEffect(() => {
    ...
    // Auto-complete the splash presentation after 1.6 seconds to let the parent's AnimatePresence handle the slide exit
    ...
    return () => {
      clearTimeout(timer);
      document.body.style.overflow = ""; // Ensure overflow is always restored
    };
  }, [onComplete]);
